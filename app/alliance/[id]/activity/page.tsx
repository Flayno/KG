  ...
  return (
    <div className="flex flex-col gap-5">
      {chartPoints.length >= 2 && (
        <Card className="p-4">
          <h2 className="text-lg font-semibold mb-2">Суммарный PvP-урон по дням</h2>
          <PowerChart points={chartPoints} color="#f783ac" />
        </Card>
      )}

      <Card className="p-2">
        <h2 className="text-lg font-semibold px-2 pt-2">Активность участников</h2>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-muted text-left border-b border-border">
                <th className="py-2 pl-2 pr-1 font-medium">Дата</th>
                <th className="py-2 px-1 font-medium">Игрок</th>
                <th className="py-2 px-1 font-medium text-right">PvP урон</th>
                <th className="py-2 px-1 font-medium text-right">Δ урона</th>
                <th className="py-2 pl-1 pr-2 font-medium text-right">Δ рейтинга</th>
              </tr>
            </thead>
            <tbody>
              {feed.map((a) => (
                <tr key={a.id} className="border-b border-border/60">
                  ...
                  <td className={`py-1.5 px-1 text-right tabular-nums ${a.diffDamage > 0 ? "text-green-400" : a.diffDamage < 0 ? "text-red-400" : "text-muted"}`}>
                    {a.diffDamage ? `${a.diffDamage > 0 ? "+" : "−"}${formatPower(Math.abs(a.diffDamage))}` : "—"}
                  </td>
                  <td className={`py-1.5 pl-1 pr-2 text-right tabular-nums ${a.dayRate > 0 ? "text-green-400" : a.dayRate < 0 ? "text-red-400" : "text-muted"}`}>
                    {a.dayRate ? `${a.dayRate > 0 ? "+" : ""}${a.dayRate.toFixed(2)}` : "—"}
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
          {feed.length === 0 && <div className="text-muted text-center py-8">Нет данных активности</div>}
        </div>
      </Card>
    </div>
  );
